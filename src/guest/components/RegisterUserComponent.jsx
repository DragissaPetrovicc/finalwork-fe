...
import { CircularProgress, TextField, FormControl, InputLabel, Input, InputAdornment, IconButton } from "@mui/material";
...
const RegisterUser = () => {
  ...
  const register = async (e) => {
    ...
    try {
      ...
      const { data } = await axiosI.post('/register/user', {firstName,lastName,username,password,location: { state, city },image, email, phoneNumber });
  ...
  return (
    <form onSubmit={register} className="w-full h-fit flex flex-col gap-4 ">
      ...
      <TextField value={firstName} onChange={(e) => setFirstName(e.target.value)} label="Firstname" variant="standard" />
      <TextField value={lastName} onChange={(e) => setLastName(e.target.value)} label="Lastname" variant="standard" />

      <TextField value={username} onChange={(e) => setUsername(e.target.value)} label="Username" variant="standard" />
